    ...
    <div class="max-w-4xl mx-auto bg-white shadow-2xl rounded-3xl p-10 border border-gray-200">
        ...
        <div class="flex items-center justify-between mb-8 border-b pb-4">
            <h1 class="text-3xl font-extrabold text-orange-500 tracking-tight">
                Editar Contacte Extern
            </h1>
            <span class="text-gray-400 text-sm">Edita les dades del contacte</span>
        </div>

        <!-- Form -->
        <form action="{{ route('external_contacts.update', $contact->id) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="tipus_servei" name="tipus_servei" required
                        ...
                        <option value="Assistencial" {{ old('tipus_servei', $contact->tipus_servei) == 'Assistencial' ? 'selected' : '' }}>Assistencial</option>
                        <option value="Servei General" {{ old('tipus_servei', $contact->tipus_servei) == 'Servei General' ? 'selected' : '' }}>Servei General</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="empresa_departament" id="empresa_departament"
                        value="{{ old('empresa_departament', $contact->empresa_departament) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="responsable" id="responsable"
                        value="{{ old('responsable', $contact->responsable) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="telefon" id="telefon"
                        value="{{ old('telefon', $contact->telefon) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="email" name="correu" id="correu"
                    value="{{ old('correu', $contact->correu) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition">
            </div>
            ...
            <div>
                ...
                <textarea id="observacions" name="observacions" rows="4"
                    class="w-full px-4 py-2 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition">{{ old('observacions', $contact->observacions) }}</textarea>
            </div>
