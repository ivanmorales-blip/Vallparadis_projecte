    ...
    <div class="overflow-x-auto">
        <table class="min-w-full bg-white shadow-lg rounded-xl border border-gray-200">
            ...
            <tbody id="services-table" class="divide-y divide-gray-200">
                @foreach ($serveis_adicionals as $index => $serveis_adicional)
                <tr class="hover:bg-orange-50 transition">

                    <!-- Clickable cells (except actions) -->
                    <td class="px-6 py-4 text-gray-600 font-medium cursor-pointer"
                        onclick="window.location='{{ route('serveis_adicionals.show', $serveis_adicional->id) }}'">
                        {{ $index + 1 }}
                    </td>
                    ...
                    <td class="px-6 py-4 flex space-x-3">
                        ...
                        <!-- Edit button -->
                        <a href="{{ route('serveis_adicionals.edit', $serveis_adicional) }}" 
                        ...
                        <form action="{{ route('serveis_adicionals.destroy', $serveis_adicional) }}" method="POST">
