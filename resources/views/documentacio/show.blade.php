...
<div class="min-h-screen bg-gray-50 p-8">
    <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg p-8 border border-gray-200 relative">
        ...
        <div class="flex justify-between items-start mb-6">
            ...
            <div class="ml-4">
                <span class="{{ $tipusColors[$document->tipus] ?? 'bg-blue-200 text-blue-800' }} px-4 py-2 rounded-full font-semibold text-sm shadow">
                    {{ $document->tipus }}
                </span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-700">
            <div>
                <p class="font-semibold">📅 Data Registrament:</p>
                <p class="mb-3">{{ \Carbon\Carbon::parse($document->data_inici)->format('d/m/Y') }}</p>

                <p class="font-semibold">👤 Professional:</p>
                <p class="mb-3">{{ $document->profesional->nom ?? '—' }} {{ $document->profesional->cognom ?? '' }}</p>
            </div>
            ...
            <div>
                ...
                <p class="mb-3">{{ $document->descripcio ?? '—' }}</p>
            </div>
        </div>
        ...
        <div class="mt-8 flex flex-wrap justify-between gap-4">
            ...
            <a href="{{ route('documentacio.edit', $document->id) }}" 
