        ...
        <table class="min-w-full bg-white shadow-lg rounded-xl border border-gray-200">
            <thead class="bg-orange-100">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left font-semibold text-gray-700 uppercase text-sm">Avaluador</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @foreach($evaluations as $evaluation)
                    <tr id="row-{{ $evaluation->id }}" class="hover:bg-orange-50 transition duration-200">
                        <td class="px-6 py-4 text-gray-600 font-medium">{{ $evaluation->id }}</td>
                        <td class="px-6 py-4 text-gray-800">{{ $evaluation->data->format('Y-m-d') }}</td>
